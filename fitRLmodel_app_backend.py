...
def simulate_fast_epsilon_greedy(params, num_trials, prob_high, prob_low, num_simulations):
    """Optimized simulation for epsilon_greedy agent."""
    ...
    q_value_history = np.zeros((num_simulations, num_trials, 2))
    ...
    for i in range(num_simulations):
        ...
        for t_pre in range(PRE_TRAIN_TRIALS):
            if random.random() < epsilon:
                ...
                # np.argmax behavior: returns first occurrence of max
                action = 0 if q_values[0] >= q_values[1] else 1
            
            reward_prob = prob_high if action == high_arm_pretrain else prob_low
        ...
        for t in range(num_trials):
            ...
            reward_prob = prob_high if action == high_arm_sim else prob_low
            reward = 1 if random.random() < reward_prob else 0
            
            q_values[action] += alpha * (reward - q_values[action])
            
            if action == high_arm_sim:
                agent_behavior[i, t] = 1
            q_value_history[i, t] = q_values
            
            if epsilon > min_epsilon:
                epsilon *= decay_rate
                
    return np.mean(agent_behavior, axis=0), np.mean(q_value_history, axis=0)
...
def load_and_prep_data(datafile, condition_str, max_trials=None): 
    ...
    try:
        ...
        raise FileNotFoundError(f"Data file '{datafile}' not found.")
    ...
    if data_condition.empty:
        ...
        raise ValueError(f"No data found for condition '{condition_str}'.")
...
def simulate_agent_with_params(model_name, params_dict, num_trials, prob_high, prob_low, num_simulations):
    ...
    """
    # OPTIMIZATION: Use fast path for epsilon_greedy
    if model_name == 'epsilon_greedy':
        return simulate_fast_epsilon_greedy(params_dict, num_trials, prob_high, prob_low, num_simulations)
...
#PLOTTING
def plot_results(mouse_curve_data, agent_curve_data, condition_str, model_name, best_params, rmse, max_trials=None, show_plot=True):
...
def plot_q_value_history(avg_q_history, prob_high, prob_low, condition_str, model_name, max_trials=None, show_plot=True): # <-- MODIFIED
    """
