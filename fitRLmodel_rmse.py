...
def load_and_prep_data(datafile, condition_str):
    ...
    print(f"Data loaded for {condition_str}.")
...
#PLOTTING
def plot_results(mouse_curve_data, agent_curve_data, condition_str, model_name, best_params, rmse):
    ...
    plt.title(f'Mouse Behavior vs. Best-Fit Simulated Agent ({model_name})')
    plt.xlabel('Trials Since Switch (blockTrial)')
    plt.ylabel('% "Good" Option Chosen')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
    
    filename = f"plot_{model_name}_{condition_str}_fit_RMSE.png" # Added RMSE to filename
    plt.savefig(filename)
    print(f"Plot saved as {filename}")
    plt.show()

#Q-VALUE HISTORY PLOTTING
def plot_q_value_history(avg_q_history, prob_high, prob_low, condition_str, model_name):
    """
    ...
    plt.title(f"Agent's Q-Value Learning Curve ({model_name}, {condition_str})")
    plt.xlabel('Trials Since Switch (blockTrial)')
    ...
    #Save the plot
    filename = f"plot_{model_name}_{condition_str}_Q_Values.png"
    plt.savefig(filename)
